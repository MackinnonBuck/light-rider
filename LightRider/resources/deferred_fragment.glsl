...
void processTrail(vec3 baseColor)
{
    vec3 trailData = texture(gColor, texCoords).xyx;
    float fragmentHeight = trailData.x;
    float fragmentTimeStamp = trailData.y;
    float noiseSeed = trailData.z;
    float currentTime = texture(gNormal, texCoords).x;
    vec3 fragmentPosition = texture(gPosition, texCoords).xyz;

    vec3 nBaseColor = normalize(baseColor) * 2.5;

    float edgeFactor = pow(abs(fragmentHeight), 5) * 2;
    float noiseFactor = clamp(cnoise(vec4(fragmentPosition * vec3(0.25, 2.5, 0.25), noiseSeed * 4)) + 1, 0, 2);
    float gradientFactor = (1 - (fragmentHeight + 1) * 0.5) * 0.75 + 0.25;
    float timeFactor = clamp((currentTime - fragmentTimeStamp) * 2.0, 0.0, 0.95);

    noiseFactor *= 0.6;
    noiseFactor = pow(noiseFactor, 8);
    noiseFactor += 0.35;
    fragColor.rgb = nBaseColor * (edgeFactor + (1 - edgeFactor) * (timeFactor * gradientFactor + (1 - timeFactor) * noiseFactor));
    //fragColor.a = (color.r + color.g + color.b) / 3;
}
...
void main()
{
	...
	else if (materialId < 1.5) // ID 1
	{
		// Ground
		fragColor = texture(gColor, texCoords).rgb;
		float lightness = (fragColor.r + fragColor.g + fragColor.b) / 3;
		lightness = pow(lightness, 4);
		lightness *= 0.5f;
		fragColor.rgb = vec3(lightness);
	}
    ...
    else if (materialId < 3.5) // ID 3
    {
        // Player 2 bike
        processBike(player2Color);
    }
    else if (materialId < 4.5) // ID 4
    {
        // Player 1 trail
        processTrail(player1Color);
    }
    else if (materialId < 5.5) // ID 5
    {
        // Player 2 trail
        processTrail(player2Color);
    }
	else
	{
		// No-op
